...
function get20s(array){
  ...
  for(let i = 0; i < array.length; i++){
    if(array[i].years.startsWith("19") && array[i].years.includes(" - 19")){
      yearsResults.push(array[i].name);
    }
  }
  return yearsResults;
}
...
function lotsOfArt(array){
  const paintingsResults = [];
  for(let i = 0; i < array.length; i++){
    if(array[i].paintings >= 100){
      paintingsResults.push(array[i].name)
    }
  }
  return paintingsResults;
}
